...
def Function(function,*args,**kwargs) :
    def wrapedFunction(*args,**kwargs) :
        try :
            ...
            log.wrapper(Function,f'''failed to execute "{functionName}" function. Received args: {args}. Received kwargs: {kwargs}''',exception)
...
def Method(method,*args,**kwargs) :
    def wrapedMethod(*args,**kwargs) :
        try :
            ...
            log.wrapper(Method,f'''failed to execute {className}{c.DOT}{methodName} method. Received args: {args}. Received kwargs: {kwargs}''',exception)
...
def overrideSignatures(toOverride, original) :
    try :
        ...
        log.wrapper(overrideSignatures,f'''failed to override signatures of {toOverride} by signatures of {original} method''',exception)
