...
def LoadSession(function,*annotatinArgs,**annotationKwargs) :
    def wraperMethod(*args,**kwargs) :
        try :
            ...
            if self.session :
                self.globals.success(f'"{self.session.key}" session loaded successfully')
            else :
                self.session = DataLoad.getBasicSession(self) ###- getBasicSession(self)
                print(f'self.session = {self.session}')
                self.globals.failure(f'''couldn't find any active session. Running most recent version of "{self.session.key}" session.''',self.globals.NOTHING)
